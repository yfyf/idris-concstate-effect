...
Note that \code{env} and \code{newenv} here environments in the \code{ConcEnv}
sense, NOT in the Effects sense, i.e. environments corresponding to the shared
resources in the \code{ConcState}.
...
\begin{itemize}
    \item \code{fork} is an \code{IO} primitive available in Idris which forks
        a program of type \code{IO ()} into a new thread.
...
run : Applicative m => Env m xs -> (prog: EffM m xs xs' a) -> m a
    \end{verbatim}
    so the \code{[env]} \emph{is} now actually an Effects \code{Env}.
\end{itemize}
...
@TODO: show and example of a "basic action", such as \code{ffork}
